...
class NCAWantedForce(Force):
    ...
    def extract_text(self) -> str:
        ...
        for container in self.soup.find_all("div", class_="most-wanted-customfields"):
            labels = container.find_all("span", class_="field-label")
            values = container.find_all("span", class_="field-value")
            for label, value in zip(labels, values):
                # Format: "Label: Value"
                lines.append(f"{label.get_text(strip=True).rstrip(':')}: {value.get_text(strip=True)}")
